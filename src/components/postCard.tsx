...
export default function PostCard({
  ...
  const isOwner = dbUserId === post.author.id;
  ...
  const handleQuantityChange = (type: "inc" | "dec") => {
    if (type === "inc" && quantity < post.stock) {
  ...
  const handleBuyNow = async () => {
    ...
    try {
      await addToCart(post.id, quantity);
      alert("Produk ditambahkan. Menuju keranjang...");
      router.push("/cart");
    } catch {
  ...
  return (
    <Card className="w-full max-w-6xl h-full p-6 shadow-lg bg-white grid md:grid-cols-12 gap-8  overflow-x-hidden">
      ...
      {/* Sidebar pembelian */}
      <div className="md:col-span-3 border w-48 border-gray-300 rounded-lg p-4 h-fit">
        ...
        <div className="flex items-center gap-3 mb-4">
          <button
            ...
            className="border border-gray-300 rounded px-1 py-1 text-lg font-semibold text-gray-600"
          >
            −
          </button>
          ...
          <button
            ...
            disabled={quantity >= post.stock}
            className="border border-green-600 text-green-600 rounded px-1 py-1 text-lg font-semibold"
          >
            +
          </button>
        ...
        <div className="flex gap-1 mb-4">
          <button
            ...
            disabled={isOwner}
            className={`font-bold rounded-lg px-1 py-1 border
      ${
        isOwner
    ...
    `}
          ...
          <button
            ...
            disabled={isOwner}
            className={`font-bold rounded-lg px-1 py-1
      ${
        isOwner
